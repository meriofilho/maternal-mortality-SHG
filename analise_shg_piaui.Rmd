...
tema_artigo <- theme_minimal(base_size = 12, base_family = fonte_base) +
  theme(
    plot.title = element_text(face = "bold", size = 13, hjust = 0, lineheight = 1.1),
...
quebrar_titulo <- function(x, largura = 45) str_wrap(x, width = largura)
```
...
perfil_resumo <- function(tabela, nome_dimensao) {
  tabela %>%
    ...
    mutate(
      pct = n / sum(n) * 100,
      !!nome_dimensao := fct_inorder(.data[[nome_dimensao]])
    )
}
...
grafico_perfil <- function(dados, dimensao, titulo, cor_barra = cor_estado) {
  ggplot(dados, aes(x = .data[[dimensao]], y = n)) +
    ...
    coord_flip(clip = "off") +
    scale_y_continuous(limits = c(0, max(dados$n) * 1.3)) +
    labs(title = quebrar_titulo(titulo), subtitle = "Piauí, 2019–2024 (n = 62)",
         x = NULL, y = "Número de óbitos",
         caption = "Fonte: MS/SVSA/CGIAE - SIM/DATASUS.") +
    tema_artigo
}

# Salva cada figura em PNG (300dpi) e PDF vetorial (fonte Roboto embutida
# via cairo_pdf) dentro de figuras/
salvar_figura <- function(grafico, nome, largura = 16, altura = 10) {
...
dist_tipo <- shg_ano %>%
  ...
  mutate(
    ...
    rotulo = fct_reorder(rotulo, Total)
  )
...
fig1 <- ggplot(dist_tipo, aes(x = rotulo, y = Total, fill = rotulo)) +
  ...
  labs(
    title = quebrar_titulo("Distribuição dos óbitos por tipo de síndrome hipertensiva"),
    subtitle = "Piauí, 2019–2024 (n = 62)",
    x = NULL, y = "Número de óbitos",
    caption = "Fonte: MS/SVSA/CGIAE - SIM/DATASUS. O11 não teve registros no período."
  ) +
  tema_artigo
...
fig2 <- ggplot(evolucao, aes(x = factor(ano), y = obitos_shg)) +
  ...
  labs(
    title = quebrar_titulo("Evolução temporal dos óbitos por SHG"),
...
perfil_raca <- perfil_resumo(shg_raca_cor, "raca_cor") %>%
  mutate(raca_cor = fct_reorder(raca_cor, n))
...
fig7 <- ggplot(rmm_valor, aes(x = factor(ano), y = rmm, fill = area, group = area)) +
  ...
  labs(
    title = quebrar_titulo("Razão de Mortalidade Materna: Piauí x Região dos Cocais"),
    subtitle = "Por 100 mil nascidos vivos — todas as causas de óbito materno",
    x = "Ano", y = "RMM (por 100 mil NV)",
    caption = "Fonte: SIM e SINASC/DATASUS. RMM não é específica de SHG (ver limitação no texto)."
  ) +
  tema_artigo
...
perfil_causa <- perfil_resumo(shg_causa_obst, "tipo_causa") %>%
  mutate(tipo_causa = fct_reorder(tipo_causa, n))
...
perfil_investigado <- perfil_resumo(shg_investigado, "situacao") %>%
  mutate(situacao = fct_reorder(situacao, n))
